...
const PH_CENTER = L.latLng(12.2795, 122.049);
const PH_ZOOM = 6;
...
export default function ResetViewControl({ position = 'topleft', padding = [20, 20] }) {
  ...
  useEffect(() => {
    ...
    control.onAdd = () => {
      ...
      const doReset = () => {
        try {
          ...
          // Fly-to animation mirroring event selection (flyTo)
          map.flyTo(PH_CENTER, PH_ZOOM, { duration: 1.0 });
        } catch (_) {}
      };

      // Mouse and keyboard activation
      L.DomEvent.on(btn, 'click', (e) => { L.DomEvent.stop(e); doReset(); });
      L.DomEvent.on(btn, 'keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') { L.DomEvent.stop(e); doReset(); }
      });
    ...
    control.addTo(map);
    return () => { try { control.remove(); } catch (_) {} };
